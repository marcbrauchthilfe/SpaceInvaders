...
bool enemies_player_hit(int player_x, int player_y, int player_width){
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        if (player_x < enemies[i].x + 10 &&
            player_x + player_width > enemies[i].x &&
            player_y < enemies[i].y + 6 &&
            player_y + 5 > enemies[i].y) {
            return true;
        }
    }
    return false;
...
void enemies_check_bullet_hits(int bullet_x, int bullet_y, bool* bullet_aktive){
    for (int e = 0; e < MAX_ENEMIES; e++) {
        ...
        if (bullet_x >= enemies[e].x &&
            bullet_x <= enemies[e].x + 10 &&
            bullet_y >= enemies[e].y &&
            bullet_y <= enemies[e].y + 6) {
